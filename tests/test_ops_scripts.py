...
class OpsScriptTests(unittest.TestCase):
    ...
    def test_opt_in_westwx_scan_is_bounded_and_failure_does_not_block_publish(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("scripts/backfill_five_minute_bc_satellite.py", calls[1])
            self.assertIn("--max-frames 2", calls[1])
            self.assertIn("--max-download-gb 0.15", calls[1])
            self.assertIn("--max-source-mb 100", calls[1])
            self.assertIn("scripts/backfill_native_bc_satellite.py", calls[2])
            self.assertIn("--max-frames 1", calls[2])
            self.assertIn("--max-download-gb 0.7", calls[2])
            self.assertIn("--max-source-mb 700", calls[2])
            self.assertIn("scripts/publish_r2.py", calls[3])
            self.assertIn("scripts/run_ingest.py", calls[4])
            self.assertIn("scripts/prune_eccc_spool.py", calls[5])
            self.assertIn("scripts/publish_r2.py", calls[6])
