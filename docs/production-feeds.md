...
Radar-Sat uses ECCC's AMQPS notification service for systematic real-time acquisition. Sarracenia receives a notification as each product is published, downloads the announced file over HTTPS, and atomically renames it into a local raw spool. The selected MSC products include the one-kilometre, ten-minute GOES-West GeoColor RGB as an independent comparison/backup to the preferred NOAA/CIRA GeoColor imagery. Separate NOAA public-S3 paths read ten-minute GOES-18 ABI imagery for North America, half-hourly GOES-18/19 ABI and Himawari-9 AHI imagery for the legacy blended products, and ten-minute GOES-18 smoke and total-lightning products.
...
The ECCC native-spool path is limited to the BC grid. Each GOES RGB GeoTIFF is read in its declared geostationary CRS, bilinearly reprojected and cropped to the EPSG:3005 grid, then encoded as quality-88 WebP. MSC GeoColor retains a 3000×2300 grid to preserve its nominal one-kilometre detail; the older ECCC backgrounds use the standard 1920×1472 grid. The one-band lightning density GeoTIFF is nearest-neighbour reprojected so isolated positive cells are not diluted; positive density bins are rendered opaque for the downstream categorical age-trail glyphs, while zero and nodata are transparent.

The parallel NOAA paths read calibrated Level-2 GOES multiband files and the northern Himawari full-disk segments with Satpy. The shared rapid path downloads each GOES-18 full-disk scan once, then renders North America and the BC raw true-colour, VisIR and enhanced-IR target grids concurrently at the native ten-minute scan clock. GOES-18 files use Google's public-data mirror first with NOAA/AWS as automatic fallback; discovery continues to use NOAA's authoritative public bucket listing. The legacy raw path keeps its GOES-18/19 North America blend and Himawari-9/GOES-18 Pacific blend at half-hour cadence; when the rapid path is enabled, it does not write a duplicate half-hour BC frame. Raw true colour and a configured 10.3 µm brightness-temperature enhancement are offered separately. The VisIR layers use true colour above +8° solar elevation, neutral grayscale 10.3/10.4 µm IR below −6°, and a smoothstep blend through twilight. They never mix the false-colour IR enhancement through the terminator, and separately suppress low-sun chroma from +1° to +12° so the source RGB cannot leave a red/yellow rim. Source scans are processed one at a time and deleted after compact WebP rasters are written.
...
At the first production sample, the compressed display layers averaged about
0.3 MB per North America frame and about 0.2 MB per North Pacific frame. The
retention policy admits 288 ten-minute-then-hourly North America times and 192
half-hour-then-hourly Pacific times per layer. The shared rapid source download
